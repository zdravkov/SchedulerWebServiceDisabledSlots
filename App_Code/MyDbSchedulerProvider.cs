...
public class MyDbSchedulerProvider : DbSchedulerProviderBase
{
    ...
    public List<DateTime> GetTimeSlots()
    { 
       List<DateTime> dates = new List<DateTime>();

       for (int i = 0; i < 5; i++)
       {
           DateTime newDate = DateTime.Now.AddHours(2*i);
           dates.Add(newDate);
       }
    ...
    public override IEnumerable<Appointment> GetAppointments(ISchedulerInfo shedulerInfo)
    {
        ...
        using (DbConnection conn = OpenConnection())
        {
            ...
            cmd.Connection = conn;
            cmd.CommandText = "SELECT [ClassID], [Subject], [Start], [End], [RecurrenceRule], [RecurrenceParentId], [Reminder] FROM [DbProvider_Classes] WHERE ([Start] < @RangeEnd AND [End] > @RangeStart) OR ([RecurrenceRule] <> '') OR ([RecurrenceParentId] IS NOT NULL)";
            cmd.Parameters.Add(CreateParameter("@RangeStart", myInfo.VisibleRangeStart));
            cmd.Parameters.Add(CreateParameter("@RangeEnd", myInfo.VisibleRangeEnd));
